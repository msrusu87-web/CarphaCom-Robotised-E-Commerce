...
const SETTINGS_FILE = "/home/ubuntu/.pni_sync_settings.json";
const CRON_FILE = "/home/ubuntu/pni_sync_cron.txt";
...
function generateCronContent(settings: SyncSettings): string {
  const lines: string[] = [
    "# === AUTO-START ===",
    "@reboot sleep 30 && /home/ubuntu/startup-services.sh",
    "",
  ...
  if (settings.stockQuickEnabled) {
    ...
    lines.push("*/" + interval + " * * * * /home/ubuntu/pni_sync_wrapper.sh stock-quick");
  } else {
  ...
  if (settings.priceStockEnabled) {
    ...
    lines.push("0 */" + hours + " * * * /home/ubuntu/pni_sync_wrapper.sh price-stock");
  } else {
  ...
  if (settings.fullImportEnabled) {
    ...
    lines.push(minute + " " + hour + " * * * /home/ubuntu/pni_sync_wrapper.sh full-import");
  } else {
