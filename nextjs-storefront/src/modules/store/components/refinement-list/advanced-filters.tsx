...
const AdvancedFilters = ({ 
  ...
  const BrandLogo = ({ brandName }: { brandName: string }) => {
    ...
    return (
      <div className="w-8 h-8 rounded-lg overflow-hidden bg-white flex items-center justify-center p-1">
        <Image
          src={`/brands/${slug}.png`}
          alt={brandName}
          width={28}
  ...
  const FilterContent = ({ isMobile = false }: { isMobile?: boolean }) => (
    <div className={clx("p-4 space-y-6", isMobile ? "pb-24" : "max-h-[calc(100vh-200px)]", "overflow-y-auto")}>
      {/* Search */}
      <div>
        <h3 className="text-xs font-semibold text-dark-400 uppercase tracking-wider mb-3">Search</h3>
      ...
      <div>
        <button 
          ...
          className="flex items-center justify-between w-full mb-3"
        >
          <h3 className="text-xs font-semibold text-dark-400 uppercase tracking-wider">Price (EUR)</h3>
          <svg 
