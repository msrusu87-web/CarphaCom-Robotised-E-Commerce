...
const HeaderSearch = ({ countryCode }: HeaderSearchProps) => {
  ...
  const fetchProducts = async () => {
    try {
      const res = await fetch(`/api/products?limit=200&countryCode=${countryCode}`)
      if (res.ok) {
      ...
      // Fallback - just navigate to store on search
    }
  }
  ...
  return (
    <div ref={wrapperRef} className="relative">
      ...
      <button
        onClick={() => setIsOpen(true)} aria-label="Search products"
      ...
      >
        ...
        <kbd className="hidden lg:inline-flex px-1.5 py-0.5 bg-dark-700 rounded text-[10px] text-dark-500">⌘K</kbd>
      </button>
      
      {/* Mobile search button */}
      <button
        onClick={() => setIsOpen(true)} aria-label="Search products"
        aria-label="Search products" className="small:hidden p-2 text-dark-400 hover:text-primary-400 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center"
      >
            ...
            <div className="bg-dark-800 border border-dark-600 rounded-2xl shadow-2xl overflow-hidden">
              ...
              <form onSubmit={handleSubmit} className="relative border-b border-dark-700">
                ...
                <input
                  ...
                  placeholder="Search CB radios, antennas, accessories..."
                ...
                {isLoading ? (
                  ...
                  results.length > 0 ? (
                    ...
                    <div className="p-6 text-center">
                      ...
                      <p className="text-dark-400">No products found for &quot;{query}&quot;</p>
                      <button
                        onClick={handleSubmit}
                  ...
                  <div className="p-6">
                    <p className="text-dark-500 text-sm mb-4">Popular searches</p>
                    <div className="flex flex-wrap gap-2">
                      {["CB Radio", "Antenna", "PNI", "Avanti", "Walkie Talkie", "Cablu"].map((term) => (
                        <button
                          key={term}
