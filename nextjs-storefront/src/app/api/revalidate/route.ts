...
const REVALIDATE_SECRET = process.env.REVALIDATE_SECRET || "YOUR_REVALIDATE_SECRET"
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (type === "products" || type === "all") {
      ...
      revalidatePath("/[countryCode]/categories/[...category]")
      results.push("All products")
    }

    if (type === "categories" || type === "all") {
      revalidateTag("categories")
      revalidatePath("/[countryCode]/categories")
      results.push("All categories")
    }

    if (type === "collections" || type === "all") {
      revalidateTag("collections")
      revalidatePath("/[countryCode]/collections/[handle]")
