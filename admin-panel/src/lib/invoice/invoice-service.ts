...
export interface CompanyInfo {
  ...
  // Tax info
  platitorTVA: boolean;
  cotaTVA: number;                 // Default VAT rate (19%)
...
export const invoiceService = {
  ...
  createInvoice(data: {
    ...
    const now = new Date();
    const dataEmitere = now.toLocaleDateString('en-US', { 
    ...
    const dataScadenta = scadenta.toLocaleDateString('en-US', { 
      day: '2-digit', month: '2-digit', year: 'numeric' 
    }).replace(/\//g, '.');
  ...
  markAsPaid(id: string): Invoice | null {
    return this.updateInvoice(id, {
      platit: true,
      dataPlatii: new Date().toLocaleDateString('en-US', { 
        day: '2-digit', month: '2-digit', year: 'numeric' 
      }).replace(/\//g, '.'),
      status: 'paid',
    });
  ...
  cancelInvoice(id: string, motiv?: string): Invoice | null {
    return this.updateInvoice(id, {
      status: 'cancelled',
      observatii: motiv ? `Cancelled: ${motiv}` : 'Cancelled',
    });
  },
  ...
  createInvoiceFromOrder(order: {
    ...
    const client: CustomerInfo = {
      ...
      tara: addr?.country_code === 'RO' ? 'Romania' : (addr?.country_code || 'Romania'),
      email: order.customer?.email,
      telefon: order.shipping_address?.phone || order.customer?.phone,
    };
    
    // Build items - Medusa unit_price is NET (without VAT) in minor units
    // pretUnitar must be GROSS (cu TVA) for the invoice template
    const tvaMultiplier = 1 + globalTVA / 100; // e.g., 1.19 for 19% TVA
    ...
    // Medusa shipping_total is GROSS (with VAT), shipping_methods[].amount is NET (without VAT)
    // Use the NET amount from shipping method and add TVA, same as product items
    const shippingMethod = order.shipping_methods?.[0];
    const shippingNetAmount = shippingMethod?.amount || 0; // NET amount in minor units
    if (shippingNetAmount > 0) {
      const shippingNet = shippingNetAmount / 100; // Convert from minor units to RON
      const shippingCuTVA = Math.round(shippingNet * tvaMultiplier * 100) / 100;
      const shippingName = shippingMethod?.name || 'Shipping';
      items.push({
        cod: 'TRANSPORT',
        denumire: `Delivery service — ${shippingName}`,
        um: 'buc',
        cantitate: 1,
  ...
  stornoInvoice(id: string, motiv?: string): Invoice | null {
    ...
    this.updateInvoice(id, {
      status: 'storno',
      observatii: motiv ? `Reversal: ${motiv}` : 'Reversed',
    });
    ...
    const stornoInvoice: Invoice = {
      ...
      status: 'confirmed',
      observatii: `Reversal invoice ${original.serie}${original.numar}${motiv ? ' - ' + motiv : ''}`,
      createdAt: now.toISOString(),
      updatedAt: now.toISOString(),
    };
