...
<head>
  ...
  <title>${invoice.tip === 'proforma' ? 'Proforma' : 'Invoice'} ${invoice.serie}${invoice.numar}</title>
  <style>
...
<body>
  ...
  <div class="invoice-title">
    <h1>
      ${invoice.tip === 'proforma' ? 'Proforma Invoice' : invoice.tip === 'chitanta' ? 'Receipt' : 'Tax Invoice'}
      ${invoice.platit ? '<span class="status-badge status-paid">PAID</span>' : 
        invoice.status === 'cancelled' ? '<span class="status-badge status-cancelled">CANCELLED</span>' :
        '<span class="status-badge status-pending">UNPAID</span>'}
    </h1>
    <div class="number">Series: <strong>${invoice.serie}</strong> No: <strong>${invoice.numar}</strong></div>
  </div>
  ...
  <table>
    <thead>
      <tr>
        ...
        <th style="width: 100px;">Unit price (incl. VAT)</th>
  ...
  <div class="payment-info">
    <h4>Bank Transfer Payment Details</h4>
  ...
  <div class="stamp-area">
    <div class="stamp-box">
      <div class="line">Supplier signature and stamp</div>
    </div>
    <div class="stamp-box">
  ...
  <div class="footer">
    <div>
      Auto-generated document | ${furnizor.website}
    </div>
    <div>
      ${invoice.eFacturaId ? `E-Factura ID: ${invoice.eFacturaId}` : 'Invoice not registered in E-Factura'}
    </div>
  </div>
...
export default { generateInvoiceHTML, generateEFacturaXML };
