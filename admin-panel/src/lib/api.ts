...
const STOREFRONT_URL = typeof window !== 'undefined' 
  ...
  : 'https://www.YOUR_PNI_USERNAMEtrafic.ro'
const REVALIDATE_SECRET = "carphatian_revalidate_2026"
...
export async function revalidateStorefront(type: 'products' | 'categories' | 'collections' | 'all', productHandle?: string) {
  try {
    // Use query param for secret (more reliable than header across origins)
...
// Customers - Admin API
export async function getAdminCustomers(offset = 0, limit = 50) {
  return adminApi(`/admin/customers?offset=${offset}&limit=${limit}`)
}
...
export async function getAdminOrders(offset = 0, limit = 20) {
  return adminApi(`/admin/orders?offset=${offset}&limit=${limit}&fields=*customer,*shipping_address,*billing_address`)
}

export async function getAdminOrderById(id: string) {
  return adminApi(`/admin/orders/${id}?fields=*customer,*shipping_address,*billing_address,*items`)
}
...
// File uploads - using custom upload API that saves to /var/www/carphacom/shared/static/
