...
export async function scanProxies(
  ...
  const pool = getPool()
  onProgress?.('Starting proxy source scanning...')
  ...
  onProgress?.(`Found ${allProxies.length} proxies from ${6} sources (${ps1.length}+${ps2.length}+${ps3.length}+${ps4.length}+${ps5.length}+${ps6.length})`)
  ...
  onProgress?.(`${unique.length} unique proxies. Saving to DB...`)
  ...
  onProgress?.(`${saved} saved. Validating first 100 proxies...`)
  ...
  for (let i = 0; i < toValidate.length; i += 20) {
    ...
    onProgress?.(`Validation: ${Math.min(i + 20, toValidate.length)}/${toValidate.length} checked, ${valid} valid`)
  }

  onProgress?.(`Scan complete: ${unique.length} scanned, ${valid} valid, ${saved} saved`)
