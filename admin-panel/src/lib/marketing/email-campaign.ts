...
const SENDER_EMAIL = 'contact@YOUR_PNI_USERNAMEtrafic.ro'
const SENDER_NAME = 'StațiiInfoTrafic'
const UNSUBSCRIBE_BASE = 'https://www.YOUR_PNI_USERNAMEtrafic.ro/api/unsubscribe'
...
export async function sendEmailCampaign(campaignId: number): Promise<{
  ...
  if (!htmlBody) throw new Error('Template has no HTML content')

  // Get all contacts from target lists (not unsubscribed)
  const listIds = campaign.list_ids || []
  if (listIds.length === 0) throw new Error('No lists selected')
  ...
  if (contacts.length === 0) throw new Error('No contacts with valid email in the selected lists')
...
function replaceVariables(text: string, contact: any): string {
  ...
  return text
    .replace(/\{\{company_name\}\}/g, contact.company_name || 'Dear Customer')
    .replace(/\{\{contact_name\}\}/g, contact.contact_name || contact.company_name || 'Dear Customer')
...
export async function generateSubjectLines(bodyPreview: string): Promise<string[]> {
  try {
    const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an email marketing expert. Generate exactly 3 email subjects in English, short (max 60 characters), catchy, without emoji. Return only the 3 subjects, one per line, without numbering.'
          },
          {
            role: 'user',
            content: `Generate 3 email subjects for this content:\n\n${bodyPreview.substring(0, 500)}`
          }
        ],
        temperature: 0.8,
        max_tokens: 200,
    ...
    if (!response.ok) return ['StațiiInfoTrafic Newsletter', 'Special offer for you', 'News and discounts']
    
    const data = await response.json()
    const text = data.choices?.[0]?.message?.content || ''
    const lines = text.split('\n').map((l: string) => l.replace(/^\d+[\.\)]\s*/, '').trim()).filter((l: string) => l.length > 5 && l.length < 100)
    
    return lines.length >= 3 ? lines.slice(0, 3) : ['StațiiInfoTrafic Newsletter', 'Special offer for you', 'News and discounts']
  } catch {
    return ['StațiiInfoTrafic Newsletter', 'Special offer for you', 'News and discounts']
  }
}
