...
async function getSMSConfig(): Promise<SMSConfig> {
  ...
  if (rows.length === 0) throw new Error('SMS is not configured. Set OVH credentials in the SMS tab.')
...
export async function sendSMS(
  ...
  return new Promise((resolve) => {
    ...
    timeout = setTimeout(() => {
      socket.destroy()
      resolve({ success: false, error: 'Timeout: SMPP connection took too long' })
    }, 30000)
...
export async function sendTestSMS(message?: string): Promise<{ success: boolean; messageId?: string; error?: string }> {
  const config = await getSMSConfig()
  const testMsg = message || `Test SMS from StațiiInfoTrafic - ${new Date().toLocaleString('en-US')}`
...
export async function sendSMSCampaign(campaignId: number): Promise<{ sent: number; failed: number; total: number }> {
  ...
  if (!campaign.sms_message) throw new Error('SMS message is empty')
  ...
  if (contacts.length === 0) throw new Error('No contacts with valid phone number')
