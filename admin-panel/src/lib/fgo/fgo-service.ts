  ...
  try {
    ...
    if (!contentType || !contentType.includes('application/json')) {
      ...
      if (text.includes('404') || text.includes('Not Found')) {
        return { 
          Success: false, 
          Eroare: 'The CUI is not registered in FGO or does not have an active subscription. Check on fgo.ro if API access is enabled.' 
        };
      }
      if (text.includes('403') || text.includes('Forbidden')) {
        return { 
          Success: false, 
          Eroare: 'Access denied. Check the API key and CUI.' 
        };
      }
      
      return { 
        Success: false, 
        Eroare: `FGO returned an invalid response (not JSON). Check the configuration.` 
      };
    }
...
export const fgoService = {
  ...
  async testConnection(): Promise<{ success: boolean; message: string; serii?: any[] }> {
    try {
      if (!fgoConfig.codUnicFurnizor) {
        return { success: false, message: 'Supplier CUI is not configured' };
      }
      ...
      return { success: false, message: 'Invalid response from FGO API' };
    } catch (error: any) {
  ...
  async createInvoiceFromOrder(order: {
    ...
    const request: EmitereFacturaRequest = {
      ...
      Explicatii: `Order #${order.display_id || order.id}`,
    };
