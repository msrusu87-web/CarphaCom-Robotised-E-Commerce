...
export async function POST(request: NextRequest) {
  try {
    ...
    // Generate XML Feed
    const baseUrl = process.env.NEXT_PUBLIC_STORE_URL || 'https://YOUR_PNI_USERNAMEtrafic.ro'
    ...
    return NextResponse.json({
      success: false,
      message: 'Error generating Google Feed',
      error: error.message
    }, { status: 500 })
  }
}
...
function generateGoogleShoppingXML(products: any[], baseUrl: string): string {
  ...
  const items = products.map(product => {
    ...
    <item>
      ...
      <g:availability>${product.status === 'published' ? 'in stock' : 'out of stock'}</g:availability>
...
<rss version="2.0" xmlns:g="http://base.google.com/ns/1.0">
  <channel>
    <title>Stații InfoTrafic - Traffic and Road Safety Products</title>
    <link>${baseUrl}</link>
    <description>Professional products for road safety and traffic management in Romania</description>
    ${items}
  </channel>
</rss>`
}
