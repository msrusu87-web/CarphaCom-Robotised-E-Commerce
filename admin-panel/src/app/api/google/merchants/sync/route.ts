...
export async function POST(request: NextRequest) {
  try {
    ...
    const authRes = await fetch('http://localhost:9000/auth/user/emailpass', {
      ...
      body: JSON.stringify({ email: 'admin@example.com', password: 'YOUR_ADMIN_PASSWORD' }),
    })
    if (!authRes.ok) throw new Error('Medusa admin auth failed')
    const { token: medusaToken } = await authRes.json()

    const medusaResponse = await fetch('http://localhost:9000/admin/products?limit=2000', {
      headers: {
    ...
    // Filter to only enabled products (those with google_merchant_enabled = true)
    ...
    if (enabledProducts.length === 0) {
      return NextResponse.json({
        ...
        failed: 0,
        total: 0,
        message: 'No products enabled for Google Merchant. Enable products from the Merchants → Products page.',
      })
    }
