  ...
  try {
    ...
    if (!config) {
      return NextResponse.json(
        { error: 'PayU is not configured or not active. Configure credentials in Admin → Store → Payments.' },
        { status: 400 }
      )
    }
    ...
    if (!config.clientId || !config.clientSecret || !config.merchantPosId) {
      return NextResponse.json(
        { error: 'PayU credentials are incomplete. Enter POS ID, Client ID and Client Secret in Admin → Store → Payments.' },
        { status: 400 }
      )
    }
    ...
    if (!cartId || !totalAmount || !customerEmail) {
      return NextResponse.json({ error: 'Required fields missing: cartId, totalAmount, customerEmail' }, { status: 400 })
    }
    ...
    const orderPayload: any = {
      notifyUrl: notifyUrl || `https://YOUR_PNI_USERNAMEtrafic.ro/app/api/payu/notify`,
      continueUrl: continueUrl || `https://YOUR_PNI_USERNAMEtrafic.ro/ro/checkout/payu-return?cartId=${cartId}`,
      customerIp: customerIp || '127.0.0.1',
      merchantPosId: config.merchantPosId,
      description: description || `Order YOUR_PNI_USERNAMEtrafic.ro`,
    ...
    return NextResponse.json(
      { error: `PayU returned status: ${result.status?.statusCode || 'UNKNOWN'}`, details: result },
      { status: 500 }
    )
  } catch (error: any) {
    console.error('PayU create order error:', error)
    return NextResponse.json(
      { error: error.message || 'Error creating PayU order' },
      { status: 500 }
    )
  }
}
...
export async function GET(request: NextRequest) {
  ...
  if (action === 'test') {
    ...
    if (!testConfig) {
      return NextResponse.json({ error: 'PayU is not configured. Enter credentials and click Save.' }, { status: 400 })
    }
    if (!testConfig.clientId || !testConfig.clientSecret) {
      return NextResponse.json({ error: 'Incomplete credentials. Enter Client ID and Client Secret, save, then test.' }, { status: 400 })
    }
    try {
      const token = await getAccessToken(testConfig)
      return NextResponse.json({
        success: true,
        message: `PayU connection ${testConfig.testMode ? '(SANDBOX)' : '(PRODUCTION)'} successful!`,
  ...
  const config = getPayUConfig()
  if (!config) {
    return NextResponse.json({ error: 'PayU is not active. Activate PayU from Admin → Store → Payments.' }, { status: 400 })
  }
