...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!sources || sources.length === 0) return NextResponse.json({ error: 'Select at least one source' }, { status: 400 })
    if (!categories || categories.length === 0) return NextResponse.json({ error: 'Select at least one category' }, { status: 400 })
