...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!campaign) return NextResponse.json({ error: 'Campaign does not exist' }, { status: 404 })
    if (campaign.status === 'sending') return NextResponse.json({ error: 'Campaign is already being sent' }, { status: 400 })
    if (campaign.status === 'sent') return NextResponse.json({ error: 'Campaign has already been sent' }, { status: 400 })
    ...
    return NextResponse.json({ success: true, message: 'Campaign sending has started' })
  } catch (err: any) {
