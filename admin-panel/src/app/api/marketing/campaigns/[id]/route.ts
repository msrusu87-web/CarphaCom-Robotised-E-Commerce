...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (rows.length === 0) return NextResponse.json({ error: 'Campaign does not exist' }, { status: 404 })
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!existing) return NextResponse.json({ error: 'Campaign does not exist' }, { status: 404 })
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (rowCount === 0) return NextResponse.json({ error: 'Campaign does not exist' }, { status: 404 })
