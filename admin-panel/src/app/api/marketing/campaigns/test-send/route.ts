...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!test_email) return NextResponse.json({ error: 'Test email required' }, { status: 400 })
    ...
    const replacements: Record<string, string> = {
      ...
      '{{unsubscribe_url}}': 'https://www.YOUR_PNI_USERNAMEtrafic.ro/api/unsubscribe?token=test-preview',
    }
    ...
    if (result.success) {
      ...
      return NextResponse.json({
        success: false,
        error: result.error || 'Error sending',
      }, { status: 500 })
    }
  } catch (err: any) {
