...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!type || !['email', 'sms'].includes(type)) return NextResponse.json({ error: 'Type must be email or sms' }, { status: 400 })
    if (!list_ids || list_ids.length === 0) return NextResponse.json({ error: 'Select at least one list' }, { status: 400 })
    ...
    const { rows } = await pool.query(
      ...
      [
        ...
        subject || '',
        sms_message || '',
        sender_email || 'contact@YOUR_PNI_USERNAMEtrafic.ro',
