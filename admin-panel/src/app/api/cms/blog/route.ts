...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await pool.query(
      ...
      [title, cleanSlug, excerpt || '', content || '', featured_image || '', author || 'Admin', status || 'draft', category || 'Guides', tags || [], seo_title || title, seo_description || excerpt || '']
    )
    ...
    if (error.code === '23505') {
      return NextResponse.json({ error: 'An article/category with this slug already exists' }, { status: 409 })
    }
    return NextResponse.json({ error: error.message }, { status: 500 })
  }
}
