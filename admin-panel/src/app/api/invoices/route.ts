...
import { generateInvoiceHTML, generateEFacturaXML } from '@/lib/invoice/invoice-template';
          ...
          const pdfHtml = html.replace(
            ...
            `<script>
              window.onload = function() {
                document.title = 'Invoice_${invoice.serie}${invoice.numar}';
  ...
  try {
    ...
    switch (action) {
      ...
      case 'saveCompany':
        ...
        return NextResponse.json({
          success: true,
          message: 'Company data has been saved',
          data: updatedCompany,
        });
      ...
      case 'create':
        ...
        return NextResponse.json({
          success: true,
          message: `Invoice ${newInvoice.serie}${newInvoice.numar} has been created`,
          data: newInvoice,
        });
      ...
      case 'createFromOrder':
        ...
        return NextResponse.json({
          success: true,
          message: `Invoice ${orderInvoice.serie}${orderInvoice.numar} has been created from order`,
          data: orderInvoice,
        });
      ...
      case 'storno':
        ...
        return NextResponse.json({
          success: true,
          message: `Reversal invoice ${stornoInvoice.serie}${stornoInvoice.numar} has been created`,
          data: stornoInvoice,
        });
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: 'Invoice has been deleted' });
  } catch (error: any) {
