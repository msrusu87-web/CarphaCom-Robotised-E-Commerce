...
function getSSLInfo(): { valid: boolean; daysLeft: number; issuer: string; expiry: string } {
  try {
    const output = exec(`echo | openssl s_client -servername YOUR_PNI_USERNAMEtrafic.ro -connect YOUR_PNI_USERNAMEtrafic.ro:443 2>/dev/null | openssl x509 -noout -dates -issuer 2>/dev/null`)
