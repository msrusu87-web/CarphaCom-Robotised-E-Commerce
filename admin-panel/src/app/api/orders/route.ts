...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'generate-invoice') {
      ...
      const orderRes = await fetch(`${MEDUSA_URL}/admin/orders/${orderId}?fields=*customer,*shipping_address,*billing_address,*items`, { headers })
      if (!orderRes.ok) return NextResponse.json({ error: 'Order not found' }, { status: 404 })
      
      const { order } = await orderRes.json()
      
      const invoiceRes = await fetch(`http://localhost:3000/app/api/invoices`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
    ...
    if (action === 'cancel') {
      ...
      if (!response.ok) {
        ...
        return NextResponse.json({ error: 'Error canceling order', details: errorData }, { status: response.status })
      }
    ...
    if (action === 'delete') {
      ...
      try {
        ...
        await pool.end()
        return NextResponse.json({ success: true, message: 'Order has been deleted' })
      } catch (dbError: any) {
        await pool.end()
