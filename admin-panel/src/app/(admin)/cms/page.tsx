...
const fmtDateTime = (d: string) => d ? new Date(d).toLocaleString('en-US', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }) : '-'
...
export default function CMSPageComponent() {
  ...
  const savePage = async (page: CMSPage) => {
    ...
    try {
      ...
      if (r.ok) { setEditingPage(null); loadPages(); showToast('Page saved') }
  ...
  const deletePage = async (id: string) => {
    ...
    loadPages(); showToast('Page deleted')
  }
  ...
  const savePost = async (post: BlogPost) => {
    ...
    try {
      ...
      if (r.ok) { setEditingPost(null); loadBlog(); showToast('Article saved') }
      else { const d = await r.json(); showToast(d.error || 'Error', 'error') }
    } catch { showToast('Error saving', 'error') }
  }
  ...
  const deletePost = async (id: string) => {
    ...
    loadBlog(); showToast('Article deleted')
  }
  ...
  const saveCategory = async (cat: BlogCategory) => {
    ...
    try {
      ...
      if (r.ok) { setEditingCategory(null); loadBlog(); showToast('Category saved') }
      else { const d = await r.json(); showToast(d.error || 'Error', 'error') }
    } catch { showToast('Error saving', 'error') }
  }
  ...
  const deleteCategory = async (id: string) => {
    ...
    loadBlog(); showToast('Category deleted')
  }
  ...
  const triggerAutoblog = async () => {
    ...
    try {
      ...
      const d = await r.json()
      showToast(d.message || 'Article generated!', d.success ? 'success' : 'error')
      loadBlog(); loadAutoblog()
    } catch { showToast('Error generating', 'error') }
  ...
  const saveAutoblogConfig = async () => {
    ...
    try {
      ...
      const d = await r.json()
      if (d.success) { setEditingAutoblogConfig(false); loadAutoblog(); showToast('Configuration saved') }
  ...
  const uploadFiles = async (files: FileList | File[]) => {
    ...
    try {
      ...
      const d = await r.json()
      if (d.success) { showToast(`${d.files?.length || 1} images uploaded`); loadMedia(mediaPage) }
      else showToast(d.error || 'Upload error', 'error')
  ...
  const deleteImage = async (id: string, source?: string) => {
    ...
    loadMedia(mediaPage); showToast('Image deleted')
  }
  ...
  const saveWidget = async (widget: Widget) => {
    ...
    try {
      ...
      if (r.ok) { setEditingWidget(null); loadWidgets(); showToast('Widget saved') }
      else { const d = await r.json(); showToast(d.error || 'Error', 'error') }
    } catch { showToast('Error saving', 'error') }
  }
  ...
  const deleteWidget = async (id: string) => {
    ...
    loadWidgets(); showToast('Widget deleted')
  }
    ...
    <div className="space-y-4 lg:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Content management, blog, media and widgets</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => {
            ...
            else if (tab === 'blog' && blogSubTab === 'posts') setEditingPost({ id: 'new-' + Date.now(), title: '', slug: '', excerpt: '', content: '', featured_image: '', author: 'Admin', status: 'draft', category: blogCategories[0]?.name || 'Guides', tags: [], seo_title: '', seo_description: '', view_count: 0, is_auto_generated: false, created_at: '', updated_at: '', published_at: '' })
      ...
      {!loading && tab === 'pages' && (
        <div className="space-y-4">
          <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
            <table className="w-full">
              <thead className="bg-gray-50 hidden lg:table-header-group">
                <tr className="text-left text-sm text-gray-500">
                  ...
                  <th className="px-6 py-3 font-medium w-36">Actions</th>
                </tr>
              </thead>
            ...
            {filteredPages.length === 0 && <div className="text-center py-8 text-gray-500">No pages found</div>}
          </div>
        </div>
      )}
      ...
      {!loading && tab === 'blog' && (
        <div className="space-y-4">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {([
              { id: 'posts' as BlogSubTab, label: 'Articles', icon: FileText, count: blogPosts.length },
              { id: 'categories' as BlogSubTab, label: 'Categories', icon: Tag, count: blogCategories.length },
              { id: 'automation' as BlogSubTab, label: 'Automation', icon: Bot },
            ]).map(st => (
          ...
          {blogStats && blogSubTab === 'posts' && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <div className="bg-purple-50 rounded-xl border border-purple-200 p-3 text-center">
                ...
                <p className="text-xs text-purple-600">Auto-generated (AI)</p>
              </div>
          ...
          {blogSubTab === 'posts' && (
            <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
              ...
              {filteredPosts.length === 0 && <div className="text-center py-8 text-gray-500">No articles found</div>}
            </div>
          )}
          ...
          {blogSubTab === 'categories' && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div key={cat.id} className="bg-white rounded-xl border p-4 hover:shadow-md transition">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-2">{cat.post_count} articles</p>
                    </div>
              ...
              {blogCategories.length === 0 && <div className="col-span-full text-center py-8 text-gray-500">No categories</div>}
            </div>
          )}
                ...
                <div className="px-6 py-4 border-b bg-gradient-to-r from-purple-50 to-blue-50 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">Automatic article generation from products &bull; {autoblogStatus.stats?.total_auto || 0} articles generated</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className={`flex items-center gap-1.5 text-sm font-medium ${autoblogStatus.cron?.status === 'active' ? 'text-emerald-600' : 'text-red-600'}`}>
                      ...
                      {autoblogStatus.cron?.status === 'active' ? 'Active' : 'Inactive'}
                    </span>
                    <button onClick={triggerAutoblog} disabled={autoblogGenerating}
                      ...
                      {autoblogGenerating ? 'Generating...' : 'Generate Now'}
                    </button>
                  </div>
                </div>
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <h4 className="text-sm font-semibold text-gray-900 flex items-center gap-2"><Cog className="w-4 h-4" /> Configuration</h4>
                      <button onClick={() => { setEditingAutoblogConfig(true); setAutoblogForm({ ...autoblogStatus.config }) }}
                        className="text-xs text-blue-600 hover:underline flex items-center gap-1"><Pencil className="w-3 h-3" /> Edit</button>
                    </div>
                    ...
                    <div>
                      <p className="text-xs font-medium text-gray-500 mb-2">Target categories</p>
                  ...
                  {/* Logs */}
                  <div className="space-y-3">
                    <h4 className="text-sm font-semibold text-gray-900 flex items-center gap-2"><Clock className="w-4 h-4" /> Generation History</h4>
                    <div className="space-y-2 max-h-[400px] overflow-y-auto">
                      ...
                      {(!autoblogStatus.recentLogs || autoblogStatus.recentLogs.length === 0) && (
                        <div className="text-center py-6 text-gray-500 text-sm">No recent logs found</div>
                      )}
                    </div>
                  </div>
                </div>
        ...
        <div className="space-y-4"
          ...
          {dragOver && (
            <div className="fixed inset-0 z-40 bg-blue-500/20 border-4 border-dashed border-blue-500 flex items-center justify-center pointer-events-none">
              <div className="bg-white rounded-2xl p-8 shadow-2xl text-center">
                ...
                <p className="text-lg font-semibold text-gray-900">Drop to upload</p>
                <p className="text-sm text-gray-500">Images will be uploaded to the media gallery</p>
              </div>
            </div>
          )}
          ...
          {mediaStats && (
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              ...
              <div className="bg-amber-50 rounded-xl border border-amber-200 p-3 text-center">
                ...
                <p className="text-xs text-amber-600">No Product</p>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-gray-500 ml-2">{mediaTotal.toLocaleString()} images</span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => loadMedia(mediaPage)} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg" title="Reload"><RefreshCw className="w-4 h-4" /></button>
            </div>
          </div>
          ...
          {mediaImages.length === 0 && !uploading && (
            <div className="bg-white rounded-xl border-2 border-dashed border-gray-300 p-12 text-center">
              ...
              <p className="text-sm text-gray-400 mt-1">Drag images here or press the button above</p>
              <button onClick={() => fileInputRef.current?.click()} className="mt-4 px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium">Upload Images</button>
            </div>
          )}
                  ...
                  <div className="aspect-square bg-gray-100 flex items-center justify-center overflow-hidden relative">
                    <img
                      ...
                      onError={e => {
                        ...
                        target.onerror = null
                        target.src = 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="200" height="200" viewBox="0 0 200 200"><rect fill="#f3f4f6" width="200" height="200"/><text fill="#9ca3af" x="100" y="100" text-anchor="middle" dy=".3em" font-size="12" font-family="sans-serif">Image unavailable</text></svg>')
                      }}
                    />
              ...
              <div className="divide-y divide-gray-100">
                {mediaImages.map(img => (
                  <div key={img.id} className="flex items-center gap-4 px-4 py-3 hover:bg-gray-50 cursor-pointer" onClick={() => setSelectedImage(img)}>
                    ...
                    {img.source === 'product' && <span className="bg-blue-100 text-blue-600 text-xs px-2 py-0.5 rounded-full shrink-0">Product</span>}
                    <span className="text-xs text-gray-400 shrink-0 hidden sm:block">{fmtDate(img.created_at)}</span>
                    <div className="flex gap-1 shrink-0">
                      <button onClick={e => { e.stopPropagation(); navigator.clipboard.writeText(img.url); showToast('URL copied') }} className="p-2 text-gray-300 hover:text-blue-600 rounded-lg"><Copy className="w-4 h-4" /></button>
          ...
          {mediaTotalPages > 1 && (
            <div className="flex items-center justify-center gap-2 pt-2">
              ...
              <span className="text-sm text-gray-500 ml-2">Page {mediaPage} / {mediaTotalPages}</span>
            </div>
          )}
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {(['header', 'hero', 'body', 'footer'] as const).map(zone => {
              ...
              const zoneLabels: Record<string, string> = { header: 'Header', hero: 'Hero', body: 'Content', footer: 'Footer' }
          ...
          <div className="bg-white rounded-xl border overflow-hidden">
            <div className="px-4 py-3 border-b bg-gray-50 flex items-center justify-between">
              ...
              <button onClick={() => setEditingWidget({ id: 'new-' + Date.now(), name: '', type: 'text', zone: activeZone, active: true, order: (widgetsByZone[activeZone]?.length || 0) + 1, content: {}, settings: {}, updated_at: '' })}
                className="text-sm text-blue-600 hover:underline flex items-center gap-1"><Plus className="w-4 h-4" /> Add Widget</button>
            </div>
            <div className="divide-y divide-gray-100">
                ...
                <div key={widget.id} className="px-4 py-3 flex items-center gap-3 hover:bg-gray-50">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <button onClick={() => toggleWidget(widget)}
                      ...
                      {widget.active ? <><ToggleRight className="w-3.5 h-3.5" /> Active</> : <><ToggleLeft className="w-3.5 h-3.5" /> Inactive</>}
                    </button>
                    <button onClick={() => setPreviewWidget(widget)} className="p-2 text-gray-400 hover:text-blue-600 rounded-lg" title="Preview"><Eye className="w-4 h-4" /></button>
                    <button onClick={() => setEditingWidget(widget)} className="p-2 text-gray-400 hover:text-green-600 rounded-lg" title="Edit"><Settings2 className="w-4 h-4" /></button>
                    <button onClick={() => deleteWidget(widget.id)} className="p-2 text-gray-400 hover:text-red-600 rounded-lg" title="Delete"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
              ))}
            </div>
            {(!widgetsByZone[activeZone] || widgetsByZone[activeZone].length === 0) && (
              <div className="text-center py-8 text-gray-500">No widgets in zone {activeZone}</div>
            )}
          </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 pt-12 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl">
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <h2 className="text-lg font-semibold">{editingPage.id.startsWith('new-') ? 'New Page' : `Edit: ${editingPage.title}`}</h2>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Template</label>
                  <select value={editingPage.template} onChange={e => setEditingPage({ ...editingPage, template: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-sm">
                    <option value="page">Standard Page</option><option value="home">Home Page</option><option value="contact">Contact</option><option value="legal">Legal/Regulations</option></select></div>
              </div>
              ...
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Content (HTML)</label>
                <textarea value={editingPage.content} onChange={e => setEditingPage({ ...editingPage, content: e.target.value })} rows={12} className="w-full px-3 py-2 border rounded-lg text-sm font-mono" /></div>
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t bg-gray-50 rounded-b-2xl">
              <button onClick={() => setEditingPage(null)} className="px-4 py-2 text-sm text-gray-700 bg-white border rounded-lg hover:bg-gray-50">Cancel</button>
              <button onClick={() => savePage(editingPage)} className="px-6 py-2 text-sm text-white bg-blue-600 rounded-lg hover:bg-blue-700 flex items-center gap-2"><Save className="w-4 h-4" /> Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      {editingPost && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 pt-12 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl">
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <h2 className="text-lg font-semibold">{!editingPost.id || editingPost.id.startsWith('new-') ? 'New Article' : `Edit: ${editingPost.title}`}</h2>
            ...
            <div className="p-6 space-y-4">
              ...
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Featured Image (URL)</label>
                <input value={editingPost.featured_image} onChange={e => setEditingPost({ ...editingPost, featured_image: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-sm" placeholder="https://..." /></div>
              ...
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Content (HTML)</label>
                <textarea value={editingPost.content} onChange={e => setEditingPost({ ...editingPost, content: e.target.value })} rows={12} className="w-full px-3 py-2 border rounded-lg text-sm font-mono" /></div>
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t bg-gray-50 rounded-b-2xl">
              <button onClick={() => setEditingPost(null)} className="px-4 py-2 text-sm text-gray-700 bg-white border rounded-lg hover:bg-gray-50">Cancel</button>
              <button onClick={() => savePost(editingPost)} className="px-6 py-2 text-sm text-white bg-blue-600 rounded-lg hover:bg-blue-700 flex items-center gap-2"><Save className="w-4 h-4" /> Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      {editingCategory && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <h2 className="text-lg font-semibold">{editingCategory.id.startsWith('new-') ? 'New Category' : 'Edit Category'}</h2>
              <button onClick={() => setEditingCategory(null)} className="p-2 hover:bg-gray-100 rounded-lg"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-6 space-y-4">
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Name</label><input value={editingCategory.name} onChange={e => setEditingCategory({ ...editingCategory, name: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-sm" /></div>
              ...
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Description</label><textarea value={editingCategory.description} onChange={e => setEditingCategory({ ...editingCategory, description: e.target.value })} rows={3} className="w-full px-3 py-2 border rounded-lg text-sm" /></div>
            </div>
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t bg-gray-50 rounded-b-2xl">
              <button onClick={() => setEditingCategory(null)} className="px-4 py-2 text-sm text-gray-700 bg-white border rounded-lg hover:bg-gray-50">Cancel</button>
              <button onClick={() => saveCategory(editingCategory)} className="px-6 py-2 text-sm text-white bg-blue-600 rounded-lg hover:bg-blue-700 flex items-center gap-2"><Save className="w-4 h-4" /> Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      {editingAutoblogConfig && autoblogForm && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 pt-12 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <h2 className="text-lg font-semibold">AutoBlog AI Configuration</h2>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Max Articles/Day</label>
              ...
              <div className="flex items-center gap-3">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm text-gray-700">Auto Publish (immediate publishing)</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={autoblogForm.is_active !== false} onChange={e => setAutoblogForm({ ...autoblogForm, is_active: e.target.checked })}
                    className="w-4 h-4 text-blue-600 rounded" />
                  <span className="text-sm text-gray-700">Generator Active</span>
                </label>
              </div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Categories (comma separated)</label>
                <input value={(autoblogForm.categories || []).join(', ')} onChange={e => setAutoblogForm({ ...autoblogForm, categories: e.target.value.split(',').map((s: string) => s.trim()).filter(Boolean) })}
                  className="w-full px-3 py-2 border rounded-lg text-sm" placeholder="Guides, Reviews, Comparisons" /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">SEO Keywords (comma separated)</label>
                <input value={(autoblogForm.keywords || []).join(', ')} onChange={e => setAutoblogForm({ ...autoblogForm, keywords: e.target.value.split(',').map((s: string) => s.trim()).filter(Boolean) })}
                  className="w-full px-3 py-2 border rounded-lg text-sm" placeholder="CB radio station, antenna, communications" /></div>
            </div>
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t bg-gray-50 rounded-b-2xl">
              <button onClick={() => setEditingAutoblogConfig(false)} className="px-4 py-2 text-sm text-gray-700 bg-white border rounded-lg hover:bg-gray-50">Cancel</button>
              <button onClick={saveAutoblogConfig} className="px-6 py-2 text-sm text-white bg-purple-600 rounded-lg hover:bg-purple-700 flex items-center gap-2"><Save className="w-4 h-4" /> Save Configuration</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">{previewWidget.type} &bull; Zone: {previewWidget.zone} &bull; {previewWidget.active ? '✅ Active' : '⛔ Inactive'}</p>
              </div>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                  ...
                  {Object.keys(previewWidget.content).length === 0 && <span className="text-xs text-gray-400">No content</span>}
                </div>
              </div>
              <div>
                ...
                <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                  ...
                  {Object.keys(previewWidget.settings).length === 0 && <span className="text-xs text-gray-400">No settings</span>}
                </div>
              </div>
              {/* Visual preview for hero-slider */}
              {previewWidget.type === 'hero-slider' && previewWidget.content.title && (
                <div>
                  <h4 className="text-sm font-semibold text-gray-700 mb-2">Visual Preview</h4>
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t bg-gray-50 rounded-b-2xl">
              <button onClick={() => { setPreviewWidget(null); setEditingWidget(previewWidget) }} className="px-4 py-2 text-sm text-blue-600 bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-100 flex items-center gap-2"><Edit className="w-4 h-4" /> Edit</button>
              <button onClick={() => setPreviewWidget(null)} className="px-4 py-2 text-sm text-gray-700 bg-white border rounded-lg hover:bg-gray-50">Close</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-3 border-b">
              <div className="min-w-0">
                ...
                <p className="text-xs text-gray-500">{selectedImage.product_title ? `Product: ${selectedImage.product_title}` : selectedImage.folder || 'Standalone image'}</p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button onClick={() => { navigator.clipboard.writeText(selectedImage.url); showToast('URL copiat') }} className="p-2 text-gray-400 hover:text-blue-600 rounded-lg" title="Copy URL"><Copy className="w-4 h-4" /></button>
            ...
            <div className="bg-gray-100 flex items-center justify-center" style={{ minHeight: '400px' }}>
              <img src={proxyImg(selectedImage.url)} alt={selectedImage.alt_text || ''} className="max-w-full max-h-[70vh] object-contain"
                onError={e => { (e.target as HTMLImageElement).src = 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="200" height="200"><rect fill="#f3f4f6" width="200" height="200"/><text fill="#9ca3af" x="50%" y="50%" text-anchor="middle" dy=".3em" font-size="14">Image unavailable</text></svg>') }} />
            </div>
            <div className="px-6 py-3 border-t bg-gray-50 text-xs text-gray-500 flex items-center justify-between">
              <span>Added: {fmtDate(selectedImage.created_at)}</span>
              <div className="flex items-center gap-3">
                {selectedImage.source && <span className={`px-2 py-0.5 rounded-full ${selectedImage.source === 'upload' ? 'bg-emerald-100 text-emerald-600' : 'bg-blue-100 text-blue-600'}`}>{selectedImage.source === 'upload' ? 'CMS Upload' : 'Product'}</span>}
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 pt-12 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h2 className="text-lg font-semibold">{widget.id.startsWith('new-') ? 'New Widget' : `Edit: ${widget.name}`}</h2>
        ...
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Zone</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex items-end pb-1"><label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-sm text-gray-700">Active (visible on site)</span></label></div>
          </div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Content (JSON)</label>
          ...
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Settings (JSON)</label>
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t bg-gray-50 rounded-b-2xl">
          <button onClick={onClose} className="px-4 py-2 text-sm text-gray-700 bg-white border rounded-lg hover:bg-gray-50">Cancel</button>
          <button onClick={handleSave} className="px-6 py-2 text-sm text-white bg-blue-600 rounded-lg hover:bg-blue-700 flex items-center gap-2"><Save className="w-4 h-4" /> Save</button>
        </div>
      </div>
    </div>
  )
