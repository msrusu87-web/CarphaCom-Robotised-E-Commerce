...
export default function SecuritatePage() {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (data.success) {
        setScanResult({ timestamp: data.timestamp, duration: data.duration, score: data.score, checks: data.checks })
        showToast(`Scan complete! Score: ${data.score}%`)
      } else {
        showToast(data.error || 'Scan error', 'error')
      }
    } catch {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500">Server and application protection — real-time data</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl p-6 border border-gray-200">
          ...
          <p className="text-sm text-gray-500">Banned IPs (Total)</p>
          <p className="text-xs text-gray-400 mt-1">{fail2ban?.currentBanned || 0} currently</p>
        </div>
        <div className="bg-white rounded-xl p-6 border border-gray-200">
        ...
        <div className="bg-white rounded-xl p-6 border border-gray-200">
          ...
          <p className="text-sm text-gray-500">SSL Days Remaining</p>
      ...
      <div className="space-y-6">
          ...
          <>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
                <h3 className="font-semibold text-gray-900">Recent Threats (Fail2Ban)</h3>
                <span className="text-xs text-gray-500">{threats.length} recent events</span>
              </div>
              {threats.length > 0 ? (
                      ...
                      <tr key={idx} className="border-t border-gray-200">
                        ...
                        <td className="px-6 py-4">
                          <span className={`flex items-center gap-1 ${threat.action === 'Ban' ? 'text-red-600' : 'text-green-600'}`}>
                            ...
                            {threat.action === 'Ban' ? 'Banned' : 'Unbanned'}
                          </span>
                        </td>
                      </tr>
                    ))}
                ...
                <div className="p-8 text-center text-gray-500">
                  ...
                  <p className="font-medium">No recent threats detected</p>
                  <p className="text-sm">Fail2ban is actively monitoring the server.</p>
                </div>
              )}
            </div>
            ...
            {connections && connections.topIPs.length > 0 && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
                  <h3 className="font-semibold text-gray-900">Top Connected IPs</h3>
                </div>
          ...
          <div className="space-y-6">
            {firewallLoading ? (
              <div className="flex items-center justify-center h-32">
                ...
                <span className="ml-3 text-gray-500">Loading firewall rules...</span>
              </div>
            ) : firewallData ? (
              <>
                {/* UFW Status */}
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                  <div className="px-6 py-4 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                      ...
                      {firewallData.ufw.status === 'active' ? '✓ Active' : '✗ Inactive'}
                    </span>
                  </div>
                  <div className="p-4">
                    ...
                    {firewallData.ufw.rules.length > 0 ? (
                      ...
                      <p className="text-gray-500 text-sm">No rules detected.</p>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                  <div className="px-6 py-4 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
                    ...
                    <span className="text-xs text-gray-500">
                      {firewallData.fail2ban.totalJails} jails · {firewallData.fail2ban.totalCurrentBanned} currently banned · {firewallData.fail2ban.totalAllTimeBanned} total
                    </span>
                  </div>
                  <div className="divide-y divide-gray-200">
                      ...
                      <div key={jail.name} className="px-6 py-4">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="flex items-center gap-4 text-sm text-gray-500">
                            ...
                            <span className="text-red-600 font-medium">{jail.currentBanned} currently banned</span>
                    ...
                    {firewallData.fail2ban.jails.length === 0 && (
                      <div className="p-6 text-center text-gray-500">
                        Fail2Ban has no configured jails.
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="p-8 text-center text-gray-500">Could not load firewall data.</div>
            )}
          </div>
        )}
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="font-semibold text-gray-900">Vulnerability Scanner</h3>
                  <p className="text-sm text-gray-500">File, port, SSL, configuration scanning — real server data</p>
                </div>
                <button
                  onClick={handleScan}
                  disabled={scanning}
              ...
              {scanResult ? (
                <>
                  <div className="grid grid-cols-3 gap-4 mb-6">
                    ...
                    <div className="p-4 bg-gray-50 rounded-lg">
                      <p className="text-sm text-gray-500">Scan Score</p>
                      ...
                      <div key={idx} className={`p-4 rounded-lg border ${
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                            ...
                            {check.status === 'pass' ? 'OK' : check.status === 'warn' ? 'Warning' : 'Danger'}
                          </span>
                        </div>
                        {check.details && check.details.length > 0 && (
                          <div className="mt-2 ml-8 space-y-1">
                            ...
                            {check.details.length > 5 && (
                              <p className="text-xs text-gray-400">... and {check.details.length - 5} more</p>
                            )}
                          </div>
                        )}
                      </div>
                ...
                <div className="p-8 text-center">
                  ...
                  <p className="text-gray-500 font-medium">No previous scan</p>
                  <p className="text-sm text-gray-400">Press the button above to run the first scan.</p>
                </div>
              )}
            </div>
          </div>
        ...
        {activeTab === "ddos" && (
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="font-semibold text-gray-900">Anti-DDoS Protection</h3>
                  <p className="text-sm text-gray-500">Connection status and server-level protection</p>
                </div>
                <span className={`px-4 py-2 rounded-full font-medium ${
                  ufw?.active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                }`}>
                  {ufw?.active ? '✓ UFW Active' : '✗ UFW Inactive'}
                </span>
              </div>
              ...
              <div className="grid grid-cols-4 gap-4 mb-6">
                ...
                <div className="p-4 bg-gray-50 rounded-lg text-center">
                  ...
                  <p className="text-sm text-gray-500">Banned IPs</p>
                </div>
                <div className="p-4 bg-gray-50 rounded-lg text-center">
                  <p className="text-2xl font-bold text-gray-900">{ufw?.rules || 0}</p>
                  <p className="text-sm text-gray-500">UFW Rules</p>
                </div>
              </div>

              <div className="space-y-3">
                <h4 className="font-medium text-gray-900">Active Protection Mechanisms</h4>
                ...
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  ...
                  <span className={`font-medium ${(fail2ban?.jails.length || 0) > 0 ? 'text-green-600' : 'text-red-600'}`}>
                    {(fail2ban?.jails.length || 0) > 0 ? `Active — ${fail2ban!.jails.length} jail(s)` : 'Inactive'}
                  </span>
                </div>
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <span className="text-gray-700">SSL/TLS Certificate</span>
                  <span className={`font-medium ${ssl?.valid ? 'text-green-600' : 'text-red-600'}`}>
                    {ssl?.valid ? `Valid — ${ssl.daysLeft} days remaining` : 'Invalid/Expired'}
                  </span>
                </div>
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
        ...
        {activeTab === "server" && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
                <h3 className="font-semibold text-gray-900">Server Security Checks</h3>
              </div>
              <div className="divide-y divide-gray-200">
                <div className="px-6 py-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {ssl?.valid
                          ? `${ssl.issuer} — Expires: ${ssl.expiry} (${ssl.daysLeft} days)`
                ...
                <div className="px-6 py-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {ufw?.active ? `Active, ${ufw.rules} rules` : 'Inactive'}
                      </p>
                    </div>
                  </div>
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                ...
                <div className="px-6 py-4 flex items-center justify-between">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {(fail2ban?.jails.length || 0) > 0
                          ? `Active, ${fail2ban!.jails.length} jails, ${fail2ban!.totalBanned} banned total`
                          : 'No active jails'}
                      </p>
                    </div>
                  </div>
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                    ...
                    {(fail2ban?.jails.length || 0) > 0 ? 'OK' : 'Warning'}
                  </span>
                </div>
                <div className="px-6 py-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                    ...
                    {(connections?.total || 0) < 500 ? 'OK' : 'Warning'}
                  </span>
                </div>
              </div>
            </div>
