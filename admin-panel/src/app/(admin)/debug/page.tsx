...
export default function DebugPage() {
  ...
  const clearLogs = async (source?: string) => {
    if (!confirm("Are you sure you want to delete the logs?")) return
    try {
      const params = new URLSearchParams()
          ...
          <>
            ...
            <div className="flex gap-2 mb-4">
              {[
                ...
                { id: "clicks", label: "Clicks", color: "purple" },
                { id: "operations", label: "Operations", color: "green" },
                { id: "revalidations", label: "Revalidations", color: "blue" },
            ...
            <div className="space-y-2 max-h-[600px] overflow-y-auto">
              {loading ? (
                ...
                <div className="text-center py-8 text-gray-500">
                  No logs {activeSubTab !== "all" ? `of type "${activeSubTab}"` : ""}
                </div>
              ) : (
        ...
        {activeTab === "live" && (
          <div>
            ...
            <div className="space-y-1 max-h-[600px] overflow-y-auto font-mono text-sm">
              {liveLogs.length === 0 ? (
                <div className="text-center py-8 text-gray-500">
                  {isLive ? "Waiting for new logs..." : "Start stream to see live logs"}
                </div>
              ) : (
                liveLogs.map(log => (
                  <div 
