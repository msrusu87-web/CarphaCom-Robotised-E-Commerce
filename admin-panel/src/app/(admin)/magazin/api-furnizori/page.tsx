...
import {
  ...
  ChevronDown, ChevronUp, Terminal, Key, Globe, Server
} from 'lucide-react'
...
export default function ApiFurnizoriPage() {
  ...
  const handleSync = async (action: string) => {
    ...
    try {
      ...
      if (json.success) {
        setMessage({ type: 'success', text: json.message || `${action} sync complete!` })
      } else {
      ...
      setMessage({ type: 'error', text: 'Error executing sync' })
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-96">
        ...
        <span className="ml-3 text-lg text-gray-600">Loading PNI data...</span>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">API Suppliers - PNI</h1>
          <p className="text-gray-500 mt-1">Manage connection, sync and MyPNI B2B configuration</p>
        </div>
        <button
        ...
        <div className={`rounded-xl border-2 p-5 ${
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <span className={`text-xs font-semibold px-2 py-1 rounded-full ${
              ...
              {tokenExpired ? 'EXPIRAT' : data?.tokenInfo?.hasToken ? 'VALID' : 'LIPSA'}
            </span>
          </div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            {!data?.tokenInfo?.hasCredentials && (
              <span className="text-xs bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full">Needs configuration</span>
            )}
          </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        ...
        <p className="text-sm text-gray-500 mb-4">Start selective sync: stock only, prices+stock, images only, or full import</p>
          ...
          {[
            ...
            return (
              ...
              >
                ...
                <h3 className="font-semibold text-gray-900 text-sm">{sync.label}</h3>
        ...
        {showSettings && (
          <div className="px-5 pb-5 border-t border-gray-100 pt-4 space-y-3">
            {[
              ...
              { key: 'image_overwrite', label: 'Overwrite Images', desc: 'Overwrite existing images (do not keep old copies)' },
            ].map((setting) => (
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-5">
        <div className="flex items-center gap-3 mb-3">
          ...
          <span className="font-medium text-gray-600 text-sm">PNI API Info</span>
        </div>
