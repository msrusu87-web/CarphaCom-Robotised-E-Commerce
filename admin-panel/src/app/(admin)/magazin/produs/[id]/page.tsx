...
export default function ProductEditPage() {
  ...
  const tabs = [
    ...
    { id: 'pricing', label: 'Prices', icon: DollarSign },
  ...
  const loadProduct = async () => {
    try {
      ...
      if (Array.isArray(meta.files)) {
        ...
        if (fileObj.userManual) fileArray.push({ name: 'User Manual', url: fileObj.userManual, type: 'manual' })
        if (fileObj.warrantyCertificate) fileArray.push({ name: 'Warranty Certificate', url: fileObj.warrantyCertificate, type: 'warranty' })
      ...
      <div className="max-w-7xl mx-auto px-4 pb-8">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <div className="space-y-6">
              ...
              {product?.metadata?.supplier && (
                <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                  <h4 className="font-medium text-blue-800 dark:text-blue-300 mb-2">Supplier Info</h4>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    ...
                    {product.metadata.imported_at && (
                      <div>
                        ...
                        <span className="ml-2 font-medium">{new Date(product.metadata.imported_at).toLocaleDateString('en-US')}</span>
                      </div>
                    )}
          ...
          {activeTab === 'images' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium">Product Images ({images.length})</h3>
              ...
              {product?.metadata?.original_images && product.metadata.original_images.length > 0 && (
                <div className="mt-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                  <h4 className="font-medium mb-3">Original Images (B2B)</h4>
          ...
          {activeTab === 'video' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium">Product Videos ({videos.length})</h3>
                <button 
                  onClick={addVideo}
            ...
            <div className="space-y-6">
              ...
              {(descriptionHtml || presentationHtml) && (
                <div className="border rounded-lg p-4 bg-gray-50 dark:bg-gray-700">
                  <h4 className="font-medium mb-3">HTML Description (from B2B)</h4>
                  <div 
                    className="prose dark:prose-invert max-w-none text-sm"
                    dangerouslySetInnerHTML={{ __html: descriptionHtml || presentationHtml || '' }}
                  />
                </div>
              )}

              <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium">Description Sections ({descriptionSections.length})</h3>
                <button 
          ...
          {activeTab === 'specifications' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium">Technical Specifications ({Object.keys(specifications).length})</h3>
              </div>
              ...
              <div className="border rounded-lg overflow-hidden">
                <table className="w-full">
                  <thead className="bg-gray-50 dark:bg-gray-700">
                    <tr>
                      <th className="px-4 py-3 text-left text-sm font-medium">Specification</th>
            ...
            <div className="space-y-6">
              ...
              {rrpPrice > 0 && distributionPrice > 0 && (
                <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                  <p className="text-sm text-blue-800 dark:text-blue-300">
                    <strong>Calculated margin:</strong> {((rrpPrice - distributionPrice) / rrpPrice * 100).toFixed(1)}% 
                    ({(rrpPrice - distributionPrice).toFixed(2)} RON profit/unit)
                  </p>
                </div>
              )}
              ...
              <div className="border rounded-lg p-4">
                  ...
                  {supplierPriceTiers.map((tier, idx) => (
                    <div key={idx} className="flex items-center gap-4 p-3 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
                      ...
                      <div className="flex items-center gap-2 flex-1">
                        ...
                        />
                        <span className="text-gray-500">RON</span>
                      </div>
                      <button 
                ...
                {supplierPriceTiers.length === 0 && (
                  <p className="text-center py-4 text-gray-500">No quantity prices</p>
                )}
              </div>
            </div>
          )}
          ...
          {activeTab === 'files' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium">Files and Documents ({files.length})</h3>
                <button 
                  onClick={addFile}
                  className="flex items-center gap-2 px-3 py-2 text-sm bg-blue-600 text-white rounded-lg"
                >
                ...
                <div key={idx} className="flex items-center gap-4 p-4 border rounded-lg">
                  ...
                  <div className="flex-1 grid grid-cols-3 gap-4">
                    ...
                    >
                      ...
                      <option value="conformity">Conformity Declaration</option>
          ...
          {activeTab === 'measurements' && (
            <div className="space-y-6">
              ...
              <div className="border rounded-lg p-4">
                ...
                <div>
                  <label className="block text-sm font-medium mb-2">Units per Box</label>
                  <input
                    type="number"
                    value={boxSize}
          ...
          {activeTab === 'connections' && (
            <div className="space-y-6">
              <div className="text-center py-12 text-gray-500">
                ...
                <p className="mb-4">Accessories and similar products</p>
                <p className="text-sm">
                  Connections are managed automatically during B2B import
                </p>
                ...
                {product?.metadata?.accessories && (product.metadata.accessories as any[]).length > 0 && (
                  <div className="mt-6 text-left border rounded-lg p-4">
                    <h4 className="font-medium mb-3">Compatible Accessories</h4>
