      ...
      <div className="flex items-center justify-center min-h-screen bg-gray-50/50">
        <div className="text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Aggregating data from all sources...</p>
          </div>
        </div>
      </div>
    )
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50/50">
        <div className="text-center space-y-4 max-w-md">
          ...
          <div>
            <p className="text-lg font-semibold text-gray-900">Loading Error</p>
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Complete store summary &bull; Updated: {lastRefresh.toLocaleTimeString('ro-RO')}
            </p>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-xs text-gray-400 hidden sm:block">
              Loaded in {d.loadTimeMs}ms
            </span>
            <button
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          <StatCard
            ...
            subtitle={`${d.products.published} published`}
            icon={Package}
            color="bg-gradient-to-br from-blue-500 to-blue-600"
            href="/magazin?tab=products"
            trend={d.products.new_this_week > 0 ? 'up' : 'neutral'}
            trendValue={d.products.new_this_week > 0 ? `+${d.products.new_this_week} this week` : 'Stable'}
          />
          <StatCard
            title="Orders"
            value={d.orders.total}
            subtitle={`${d.orders.pending} pending`}
            icon={ShoppingCart}
            color="bg-gradient-to-br from-amber-500 to-orange-500"
            href="/magazin?tab=orders"
            trend={d.orders.today > 0 ? 'up' : 'neutral'}
            trendValue={d.orders.today > 0 ? `+${d.orders.today} today` : `${d.orders.this_week} this week`}
          />
          <StatCard
            title="Revenue"
            ...
            trend="up"
            trendValue={`${d.orders.total} orders`}
          />
          <StatCard
            title="Customers"
            value={d.customers.total}
            subtitle={d.customers.new_this_month > 0 ? `+${d.customers.new_this_month} this month` : 'Active'}
            icon={Users}
            color="bg-gradient-to-br from-purple-500 to-violet-600"
            href="/magazin?tab=customers"
            trend={d.customers.new_this_week > 0 ? 'up' : 'neutral'}
            trendValue={d.customers.new_this_week > 0 ? `+${d.customers.new_this_week} new` : 'Stable'}
          />
          <StatCard
            title="Categories"
            value={d.categories.total}
            subtitle={`${d.categories.top_level} top-level`}
            icon={Layers}
          ...
          <StatCard
            title="Blog"
            value={d.blog.posts}
            subtitle={`${d.blog.published_posts} published`}
            icon={FileText}
            color="bg-gradient-to-br from-pink-500 to-rose-500"
            href="/blog"
            trend={d.blog.auto_generated > 0 ? 'up' : 'neutral'}
            trendValue={`${d.blog.auto_generated} auto-generated`}
          />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">{d.orders.pending} pending &bull; {d.orders.total} total</p>
              </div>
                  ...
                  <div key={order.id} className="px-6 py-3.5 hover:bg-gray-50/50 transition-colors flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 truncate">
                        {order.email} &bull; {order.itemCount} products &bull; {new Date(order.createdAt).toLocaleDateString('ro-RO', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}
                      </p>
                    </div>
            ...
            {d.recentOrders.length > 0 && (
              <div className="px-6 py-3 bg-gray-50/50 border-t border-gray-100">
                <div className="flex items-center justify-between text-xs text-gray-500">
                  <span>Total Revenue: <strong className="text-gray-900">{d.revenue.total.toLocaleString('ro-RO', { minimumFractionDigits: 2 })} lei</strong></span>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div className="p-3 space-y-1">
              {[
                { label: 'Add Product', desc: 'New product in catalog', icon: Plus, color: 'text-blue-600 bg-blue-50', href: '/magazin?tab=products' },
                { label: 'Manage Orders', desc: 'Accept and process', icon: ShoppingCart, color: 'text-amber-600 bg-amber-50', href: '/magazin?tab=orders' },
                { label: 'Generate Invoice', desc: 'Internal invoicing', icon: Receipt, color: 'text-emerald-600 bg-emerald-50', href: '/facturare' },
                { label: 'Import CSV/XML', desc: 'Upload products in bulk', icon: FileSpreadsheet, color: 'text-purple-600 bg-purple-50', href: '/magazin?tab=apis' },
                { label: 'Blog Article', desc: 'Create new content', icon: Edit, color: 'text-pink-600 bg-pink-50', href: '/blog' },
                { label: 'SEO Settings', desc: 'Search engine optimization', icon: Search, color: 'text-teal-600 bg-teal-50', href: '/seo' },
              ].map((action) => (
                <Link
                  key={action.label}
                  href={action.href}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <Link href="/magazin?tab=products" className="text-xs text-blue-600 hover:text-blue-700 font-medium">Manage &rarr;</Link>
            </div>
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-blue-50 rounded-xl p-3 text-center">
                  <p className="text-2xl font-bold text-blue-700">{Number(d.products.total).toLocaleString()}</p>
                  <p className="text-xs text-blue-500 mt-0.5">Products</p>
                </div>
                <div className="bg-amber-50 rounded-xl p-3 text-center">
                  <p className="text-2xl font-bold text-amber-700">{d.categories.total}</p>
                  <p className="text-xs text-amber-500 mt-0.5">Categories</p>
                </div>
              </div>
              ...
              <div className="space-y-2.5">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600 flex items-center gap-1.5">
                    <div className="w-2 h-2 rounded-full bg-blue-500" /> API Products (B2B)
                  </span>
                  <span className="font-semibold text-gray-900">{Number(d.products.from_b2b).toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600 flex items-center gap-1.5">
                    <div className="w-2 h-2 rounded-full bg-purple-500" /> CSV/XML Products
                  </span>
                  <span className="font-semibold text-gray-900">{d.products.from_csv}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600 flex items-center gap-1.5">
                    <div className="w-2 h-2 rounded-full bg-emerald-500" /> Manual Products
                  </span>
              ...
              <div className="grid grid-cols-3 gap-2 pt-1">
                ...
                <div className="text-center p-2 bg-gray-50 rounded-lg">
                  ...
                  <p className="text-xs text-gray-500">Img/Product</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <Link href="/magazin?tab=inventory" className="text-xs text-blue-600 hover:text-blue-700 font-medium">Details &rarr;</Link>
            </div>
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-emerald-50 rounded-xl p-3 text-center">
                  <p className="text-2xl font-bold text-emerald-700">{Number(d.products.total_stock).toLocaleString()}</p>
                  <p className="text-xs text-emerald-500 mt-0.5">Stock Units</p>
                </div>
              ...
              <ProgressBar
                ...
                max={d.products.total}
                color="bg-amber-500"
                label={`Low stock (<=5): ${d.products.low_stock} products`}
              />
              ...
              {d.stockAlerts.length > 0 && (
                <div className="space-y-2 pt-2">
                  <p className="text-xs font-medium text-red-600 flex items-center gap-1">
                    <AlertTriangle className="w-3.5 h-3.5" /> Stock Alert
                  </p>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <Link href="/facturare" className="text-xs text-blue-600 hover:text-blue-700 font-medium">Manage &rarr;</Link>
            </div>
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-indigo-50 rounded-xl p-3 text-center">
                  <p className="text-2xl font-bold text-indigo-700">{d.invoices.count}</p>
                  <p className="text-xs text-indigo-500 mt-0.5">Invoices Issued</p>
                </div>
                <div className="bg-violet-50 rounded-xl p-3 text-center">
                  <p className="text-2xl font-bold text-violet-700">
                    {d.invoices.totalValue > 0 ? `${(d.invoices.totalValue / 100).toLocaleString('ro-RO', { maximumFractionDigits: 0 })}` : '0'}
                  </p>
                  <p className="text-xs text-violet-500 mt-0.5">Value (lei)</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <Link href="/facturare" className="flex items-center gap-3 p-3 bg-indigo-50 rounded-xl hover:bg-indigo-100 transition-colors group">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium text-indigo-900">Issue New Invoice</p>
                    <p className="text-xs text-indigo-600">Proforma or fiscal invoice</p>
                  </div>
              ...
              {d.invoices.recent && d.invoices.recent.length > 0 && (
                <div className="space-y-2 pt-1">
                  <p className="text-xs font-medium text-gray-500">Latest Invoices</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div className="p-5 space-y-3">
              ...
              <div className="space-y-1.5">
                <Link href="/blog" className="flex items-center gap-2 p-2.5 bg-pink-50 rounded-lg hover:bg-pink-100 transition-colors text-sm font-medium text-pink-700">
                ...
                <Link href="/cms" className="flex items-center gap-2 p-2.5 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors text-sm font-medium text-gray-700">
                  <Settings className="w-4 h-4" /> CMS Management
                </Link>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div className="p-5 space-y-3">
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">Indexable products</span>
                  <span className="font-bold text-gray-900">{Number(d.products.published).toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">Blog pages</span>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div className="p-5 space-y-3">
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">Eligible products</span>
                  <span className="font-bold text-gray-900">{Number(d.products.published).toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">With image</span>
                ...
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">Without image</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <Link href="/debug" className="text-xs text-blue-600 hover:text-blue-700 font-medium">Diagnostics &rarr;</Link>
            </div>
            <div className="p-5 space-y-5">
                ...
                {d.system.memory && (
                  <div>
                    <p className="text-xs font-medium text-gray-500 mb-2 flex items-center gap-1">
                      <Cpu className="w-3 h-3" /> RAM Memory
                    </p>
              ...
              {d.system.pm2 && d.system.pm2.length > 0 && (
                <div>
                  <p className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-2">Processes (PM2)</p>
                  <div className="space-y-1.5">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              ...
              <div className="p-5">
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { label: 'SSL/HTTPS', status: true, desc: 'Active' },
                    { label: 'Admin Auth', status: true, desc: 'Active' },
                    { label: 'Firewall', status: true, desc: 'Active' },
                    { label: 'Auto Backup', status: false, desc: 'To configure' },
                  ].map((item) => (
                    <div key={item.label} className={`flex items-center gap-2.5 p-3 rounded-xl ${item.status ? 'bg-emerald-50' : 'bg-amber-50'}`}>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                ...
                <Link href="/magazin?tab=products" className="text-xs text-blue-600 hover:text-blue-700 font-medium">All &rarr;</Link>
              </div>
              <div className="divide-y divide-gray-50">
        ...
        <div className="text-center py-4 border-t border-gray-100">
          <p className="text-xs text-gray-400">
            CarphasCom Admin Panel &bull; Dashboard generated at {new Date(d.timestamp).toLocaleString('ro-RO')} &bull; {d.loadTimeMs}ms
          </p>
        </div>
      </div>
    </div>
